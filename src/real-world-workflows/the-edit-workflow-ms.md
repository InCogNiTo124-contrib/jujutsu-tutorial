...
Let's create support for providing multiple files through CLI arguments. For that, edit `bak.py` to have the following content:
...
def main():
    ...
    filenames = sys.argv[1:]  # assume bak.py is followed by list of filenames
...
We've changed about half of our lines, but most of it is indendation. Commit the contents of the working copy with `jj commit`.
