...
BAK_V2 = Path("bak/bak_v2.py").read_text()
BAK_V2_REFACTOR = Path("bak/bak_v2_refactor.py").read_text()
BAK_V2_ASSERT = Path("bak/bak_v2_assert.py").read_text()
...
def setup(temp_dir: str):
    ...
    run_command("jj describe --no-edit --reset-author", temp_dir)
...
def jj_new(temp_dir: str, before: str = None) -> Commit:
    cmd_str = "jj new"
    if before is not None:
        cmd_str += f" -B {before}"
    _, status = run_command(cmd_str, temp_dir)
    lines = status.split("\n")
...
def main():
    ...
    while True:
        ...
        with (Path(temp_dir.name) / "bak.py").open("w") as file:
            file.write(BAK_V2)
        _, commit4 = jj_describe("Add support for arguments", temp_dir.name)

        # 3.2
        _ = jj_new(temp_dir.name)
        with (Path(temp_dir.name) / "bak.py").open("w") as file:
            file.write(BAK_V2_REFACTOR)
        _, commit6 = jj_describe("Refactor the code", temp_dir.name)

        # 3.3
        _ = jj_new(temp_dir.name, before = commit6.change_id)
        with (Path(temp_dir.name) / "bak.py").open("w") as file:
            file.write(BAK_V2_ASSERT)
        _, commit5 = jj_describe("Add missing filesystem checks", temp_dir.name)
    ...
    run_command(f"mv '{temp_dir.name}' /tmp/bak")
